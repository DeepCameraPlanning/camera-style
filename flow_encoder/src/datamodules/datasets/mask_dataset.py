...
class MaskFlowDataset(Dataset):
    """Load a temporally and/or spatially masked sample from precomputed flows.
    ...
    :param n_temporal_mask: number of frame to keep (not masked).
    :param spatial_mask_size: size of the of the frame to keep (not masked).
    """
    ...
    def __init__(
        ...
        self._n_temporal_mask = n_temporal_mask
        self._spatial_mask_size = spatial_mask_size
    ...
    def _split_chunks(array: List[Any], stride: int, chunk_size: int):
        """successive n-sized chunks from `array`."""
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        masked_flows = torch.zeros_like(flows)
        # masked_flows[:, self._n_temporal_mask :] = flows[
        #     :, self._n_temporal_mask :
        # ]
        masked_flows[:, :, 102:122, 102:122] = flows[:, :, 102:122, 102:122]
