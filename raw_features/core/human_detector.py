...
from .utils_movie_style import compute_bbox_iou, get_patches
...
class HumanDetector:
    ...
    def __init__(
        ...
        self.trainer = Trainer(
            gpus=num_gpus,
            num_nodes=num_nodes,
            strategy="dp",  # accelerator->strategy according to new paradigm
            logger=False,
            enable_progress_bar=False,  # new paradigm too
    ...
    def _preprocess_frames(self, frames: List[np.array]) -> List[torch.Tensor]:
        ...
        for frame in frames:
            ...
            processed_frames.append({"image": image, "height": height, "width": width})
    ...
    def detect_human(self, frames: List[np.array]) -> List[np.array]:
        """
    ...
    @staticmethod
    def percentage_surface(box, h, w):
        # xmin, ymin, xmax, ymax
        surface = (box[2] - box[0]) * (box[3] - box[1])
        surface = surface / (w * h)
        return surface

    @staticmethod
    def track_biggest(bbox_tracks, H, W, surface_threshold: float = 0.05):

        # decide on first frame
        bbox_tracks_big = [
            bbox_track
            for bbox_track in bbox_tracks
            if HumanDetector.percentage_surface(bbox_track[0][1:], H, W)
            > surface_threshold
        ]

        # print("len", len(bbox_tracks_big))
        if len(bbox_tracks_big) != 0:
            bbox_track_longest_big = max(
                bbox_tracks_big,
                key=lambda x: len(x) * (x[0][2] - x[0][0]) * (x[0][3] - x[0][1]),
            )
        else:
            bbox_track_longest_big = []
        return bbox_track_longest_big

    def detect_and_track_biggest_human(
        self, frames: List[np.array], surface_threshold: float = 0.05
    ) -> List[np.array]:
        H, W, C = frames[0].shape
        bboxes, masks, scores = self.detect_human(frames)
        bbox_tracks = self.get_bboxtracks(bboxes, scores)

        print("len bbox_tracks", len(bbox_tracks))
        bbox_track_longest_big = HumanDetector.track_biggest(
            bbox_tracks, H, W, surface_threshold
        )
    ...
    def get_peoplefeatures(
        ...
        for frame_index, frame in enumerate(frames):
            bbox_patches = get_patches(np.zeros(frame.shape[:2]), (n_row, n_col))
    ...
    def match_bbox_annotation(
        ...
        for track in bbox_tracks:
            ...
            if track[box_index].size > 0:
                ...
                iou = compute_bbox_iou(track[box_index][:, -4:], annotated_bbox)
                ious.append(iou[0])
            else:
                ious.append(-1)
        ...
        for bbox_index in range(len(track)):
            ...
            if (bbox_index > 0) and (
                ...
                for missing_index in range(last_index + 1, frame_index):
                    alpha = (frame_index - missing_index) / (frame_index - last_index)
...
class NaiveTracker(Tracker):
    ...
    def _merge_tracks(
        ...
        for f_track_index, f_track in enumerate(forward_tracks):
            ...
            for b_track_index, b_track in enumerate(backward_tracks):
                ...
                if not (f_track_index in f_visited) or not (b_track_index in b_visited):
                    ...
                    if n_common_bboxes >= common_threshold:
                        ...
                        merged_tracks.append(
                            merged[
                                merged.groupby("frame_index")["score"].transform(max)
                                == merged["score"]
                            ]
            ...
            for track_index, reference_track in enumerate(pending_tracks):
                ...
                if possible_match.size > 0:
                    ...
                    pending_tracks[track_index].insert(
                        order,
                        np.array([frame_index, best_score] + best_bbox.tolist()),
                    )
                    current_bboxes = np.delete(current_bboxes, best_bbox_index, axis=0)
                    current_scores = np.delete(current_scores, best_bbox_index, axis=0)

                # No matching bbox, check if the reference track is over
                else:
                    if abs(frame_index - ref_frame_index) >= self.max_frame_threshold:
    ...
    def _process_tracks(
        ...
        for track_index, track in enumerate(bbox_tracks):
            ...
            # Interpolate missing bounding-boxes
            active_track = self.interpolate_bboxes(track, first_frame, last_frame)
